...
export default function AuthPage({ mode }: AuthPageProps) {
  ...
  const handleDemoLogin = async (demoRole: "candidate" | "recruiter") => {
    ...
    const { login } = await import("@/features/auth/store/authSlice");
    const { MOCK_CANDIDATE, MOCK_RECRUITER } = await import("@/lib/mockData");
    if (demoRole === "recruiter") {
      dispatch(login({ email: "hr@abc.com", password: "12345678" })).then(() =>
        redirect("recruiter-dashboard"),
      );
    } else {
      dispatch(
        login({ email: "aryan.sharma@email.com", password: "12345678" }),
      ).then(() =>
        redirect("candidate-dashboard"),
      );
    }
    // redirect(
    //   demoRole === "recruiter" ? "recruiter-dashboard" : "candidate-dashboard",
    // );
  };
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-blue-700 to-blue-900 flex-col justify-between p-12">
        ...
        <div>
          ...
          <p className="text-blue-200 text-lg mb-10 leading-relaxed">
            Join 2 million+ professionals who use TalentHub to advance their
            careers.
          </p>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-5 border border-white/20">
          <p className="text-sm text-blue-100 italic mb-3">
            "TalentHub helped me land a 40% salary hike at Google. The AI
            matching is eerily accurate."
          </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(["candidate", "recruiter"] as const).map((r) => (
                  ...
                  >
                    ...
                    <p
                      className={`text-sm font-semibold capitalize ${role === r ? "text-blue-700" : "text-gray-700"}`}
                    >
                      {r}
                    </p>
